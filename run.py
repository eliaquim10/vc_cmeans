...
# %%
y_image = (model.G[y].reshape(image.shape)*255).astype(np.uint8)
...
img_name = "./results/comics/imgs/20/"
# os.mkdir(img_name)

model_name = "./results/comics/model/20/"
# os.mkdir(model_name)

Image.fromarray(y_image).save(f"{img_name}{filename}")
model.save_model(f"{model_name}model.json")
...
img_name = "./results/comics/imgs/10/"
# os.mkdir(img_name)

model_name = "./results/comics/model/10/"
# os.mkdir(model_name)
...
img_name = "./results/art/imgs/20/"
# os.mkdir(img_name)

model_name = "./results/art/model/20/"
# os.mkdir(model_name)
...
img_name = "./results/art/imgs/10/"
# os.mkdir(img_name)

model_name = "./results/art/model/10/"
# os.mkdir(model_name)
...
# %%
import pandas as pd
pd.DataFrame(np.array(metricas_resultados).T, index=["ssim histrograma", "ssim", "uqi"]).T.to_csv(f"./results/metricas.csv")
